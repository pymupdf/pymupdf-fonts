...
If the package is installed, PyMuPDF supports its fonts automatically. To use one, execute ``font = fitz.Font("code")``, where ``code`` is a value from the first table column below.

More detail is contained in the PyMuPDF documentation of the [Font](https://pymupdf.readthedocs.io/en/latest/font.html) class.
